...
export const fetchAsyncNewPost = createAsyncThunk(
  ...
  async (newPost: PROPS_NEWPOST) => {
    ...
    // linkedは指定しなければバックエンドのシリアライザが勝手に空配列を割り当てる？
    ...
    return res.data; // 新規作成したPostのオブジェクト
  }
);
...
export const fetchAsyncPatchLiked = createAsyncThunk(
  ...
  async (liked: PROPS_LIKED) => {
    ...
    let isOverlapped = false; // 新規いいね、か、いいねの削除か
    ...
    if (!isOverlapped) {
      ...
      // 変更前のいいねが1件のみ、かついいねを外そうとしている
      uploadData.append("title", liked.title);
      // patchではlikedを空配列に更新できないからputでまるごと更新
      ...
      return res.data; // 更新後のPostオブジェクト
    }
